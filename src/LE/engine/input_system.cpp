...
void input_system::update_keystates()
{
  ...
  p_curr_mouse_pos.set((unsigned)x, (unsigned)y);
  ...
  {
    p_mouse_wheel_delta = uvec2::zero;
  }
}
...
uvec2 const& input_system::get_mouse_pos() const
{
  return p_curr_mouse_pos;
}

uvec2 const& input_system::get_prev_mouse_pos() const
{
  return p_prev_mouse_pos;
}

uvec2 input_system::get_delta_mouse_pos() const
{
  return p_curr_mouse_pos - p_prev_mouse_pos;
}

void input_system::set_mouse_wheel_delta(uvec2 const& new_delta)
{
  p_mouse_wheel_set_this_update = true;
  p_mouse_wheel_delta = new_delta;
}

uvec2 input_system::get_mouse_wheel_delta() const
{
